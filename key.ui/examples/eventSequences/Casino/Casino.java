...
class Casino {
    ...
    /*@ normal_behavior
      ...
      @ ensures \result == true && (*!eventSeq(seqConcat(seqSingleton(\if(event(Casino_decideBet_int, self.cond1))\then(TRUE)\else(FALSE)), seqSingleton(\if(event(Casino_createGame_int, self.cond2))\then(TRUE)\else(FALSE))))*);
      @ assignable \everything;
      @*/
    public boolean start(Address operator, Address player, int money, Coin guess) {
